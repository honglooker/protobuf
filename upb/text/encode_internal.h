...
UPB_INLINE void UPB_PRIVATE(txtenc_putbytes)(txtenc* e, const void* data,
                                             size_t len) {
...
UPB_INLINE void UPB_PRIVATE(txtenc_putstr)(txtenc* e, const char* str) {
  UPB_PRIVATE(txtenc_putbytes)(e, str, strlen(str));
}

UPB_INLINE void UPB_PRIVATE(txtenc_printf)(txtenc* e, const char* fmt, ...) {
  size_t n;
  size_t have = e->end - e->ptr;
  va_list args;
...
UPB_INLINE void UPB_PRIVATE(txtenc_indent)(txtenc* e) {
...
UPB_INLINE void UPB_PRIVATE(txtenc_endfield)(txtenc* e) {
...
UPB_INLINE void UPB_PRIVATE(txtenc_escaped)(txtenc* e, unsigned char ch) {
  switch (ch) {
    case '\n':
      UPB_PRIVATE(txtenc_putstr)(e, "\\n");
      break;
    case '\r':
      UPB_PRIVATE(txtenc_putstr)(e, "\\r");
      break;
    case '\t':
      UPB_PRIVATE(txtenc_putstr)(e, "\\t");
      break;
    ...
    case '\\':
      ...
      break;
    default:
      UPB_PRIVATE(txtenc_printf)(e, "\\%03o", ch);
      break;
  }
}
...
UPB_INLINE bool UPB_PRIVATE(upb_DefinitelyNeedsEscape)(unsigned char ch) {
...
UPB_INLINE bool UPB_PRIVATE(upb_AsciiIsPrint)(unsigned char ch) {
  return ch >= 32 && ch < 127;
}

// Returns true if this is a high byte that requires UTF-8 validation.  If the
// UTF-8 validation fails, we must escape the byte.
UPB_INLINE bool UPB_PRIVATE(upb_NeedsUtf8Validation)(unsigned char ch) {
  return ch > 127;
}
...
UPB_INLINE size_t UPB_PRIVATE(SkipPassthroughBytes)(const char* ptr,
                                                    size_t size) {
  for (size_t i = 0; i < size; i++) {
    unsigned char uc = ptr[i];
    if (UPB_PRIVATE(upb_DefinitelyNeedsEscape)(uc)) return i;
    if (UPB_PRIVATE(upb_NeedsUtf8Validation)(uc)) {
      ...
      size_t end = i + 1;
      for (; end < size; end++) {
        if (!UPB_PRIVATE(upb_NeedsUtf8Validation)(ptr[end])) break;
      }
      size_t n = end - i;
...
UPB_INLINE void UPB_PRIVATE(upb_HardenedPrintString)(txtenc* e, const char* ptr,
                                                     size_t len) {
  ...
  while (ptr < end) {
    size_t n = UPB_PRIVATE(SkipPassthroughBytes)(ptr, end - ptr);
    if (n != 0) {
      UPB_PRIVATE(txtenc_putbytes)(e, ptr, n);
      ptr += n;
      if (ptr == end) break;
    }
    ...
    UPB_PRIVATE(txtenc_escaped)(e, *ptr);
    ptr++;
  }
  UPB_PRIVATE(txtenc_putstr)(e, "\"");
}

UPB_INLINE void UPB_PRIVATE(txtenc_bytes)(txtenc* e, upb_StringView data) {
  const char* ptr = data.data;
  const char* end = ptr + data.size;
  UPB_PRIVATE(txtenc_putstr)(e, "\"");
  for (; ptr < end; ptr++) {
    unsigned char uc = *ptr;
    if (UPB_PRIVATE(upb_AsciiIsPrint)(uc)) {
      UPB_PRIVATE(txtenc_putbytes)(e, ptr, 1);
    } else {
      UPB_PRIVATE(txtenc_escaped)(e, uc);
    }
  }
  UPB_PRIVATE(txtenc_putstr)(e, "\"");
}

UPB_INLINE size_t UPB_PRIVATE(txtenc_nullz)(txtenc* e, size_t size) {
