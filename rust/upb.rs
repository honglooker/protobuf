...
pub unsafe fn upb_textencode_debug(
    msg: RawMessage,
    ...
    let real_size: usize = unsafe { upb_TextEncode_Debug(msg, mt, options, buf, size) };
    if real_size < size {
        ...
        let mut buf2_vec: Vec<u8> = Vec::with_capacity(real_size);
        let buf2: *mut u8 = buf2_vec.as_mut_ptr();
        let real_size2: usize = unsafe { upb_TextEncode_Debug(msg, mt, options, buf2, real_size) };
        let slice2 = unsafe { slice::from_raw_parts(buf2, real_size2) };
        match std::str::from_utf8(slice2) {
            Ok(s) => s.to_string(),
            Err(_e) => "Failed to encode message as text".to_string(),
        }
    }
