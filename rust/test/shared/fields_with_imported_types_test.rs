...
fn test_oneof_message_field_generated() {
    ...
    assert_that!(
        ...
        matches_pattern!(msg_with_fields_with_imported_types::ImportedTypesOneof_Oneof::not_set(_))
    );
    assert_that!(
        msg.imported_types_oneof_case(),
        matches_pattern!(msg_with_fields_with_imported_types::ImportedTypesOneof_Case::not_set)
    );
}
